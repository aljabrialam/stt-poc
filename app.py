...
comprehend_client = boto3.client('comprehend', region_name=st.secrets["AWS_REGION"])
...
def transcribe_audio(file_name, file_format):
    ...
    try:
        transcribe_client.start_transcription_job(
            ...
            MediaFormat=file_format,  # Use the file format (wav or mp3)
            LanguageCode='en-US'
        )
...
# Function to analyze text using Amazon Comprehend
def analyze_text(text):
    try:
        entities = comprehend_client.detect_entities(Text=text, LanguageCode='en')
        event_details = {}
        
        # Extract relevant entities and organize them into a dictionary
        for entity in entities['Entities']:
            entity_type = entity['Type']
            entity_text = entity['Text']
            if entity_type not in event_details:
                event_details[entity_type] = []
            event_details[entity_type].append(entity_text)
        
        return event_details, entities  # Return both the processed details and the raw JSON
    except (ClientError, Exception) as e:
        return str(e), None

# Function to summarize transcript text using Bedrock's Claude model
...
if audio_base64:
    ...
    with st.spinner("Transcribing recorded audio..."):
        transcript = transcribe_audio(file_name, 'wav')
    ...
    if transcript:
        ...
        # Analyze the transcript with Amazon Comprehend
        if st.button("Analyze Transcript with Comprehend"):
            with st.spinner("Analyzing text..."):
                entities, raw_response = analyze_text(transcript)
...
st.markdown("## Upload a Recorded Audio File for Transcription and Summarization")
...
if uploaded_file:
    ...
    file_name = f"audio/{uuid.uuid4()}.{uploaded_file.name.split('.')[-1]}"
    upload_to_s3(audio_bytes, file_name)

    # Transcribe the uploaded file
    with st.spinner("Transcribing uploaded audio..."):
        transcript = transcribe_audio(file_name, uploaded_file.name.split('.')[-1])
    ...
    if transcript:
        ...
        if st.button("Analyze Transcript with Comprehend"):
            with st.spinner("Analyzing text..."):
                ...
                st.markdown("""
                    <h2>Information to be Extracted:</h2>
                    <p>The following types of information will be extracted from the transcript:</p>
                    <ul>
                        <li><strong>Event name or person name</strong></li>
                        <li><strong>Date and time</strong></li>
                        <li><strong>Location</strong></li>
                        <li><strong>Contact information</strong> (an email address, website, etc.)</li>
                        <li><strong>Performers or speakers</strong> who will attend</li>
                        <li><strong>Event type</strong> (conference, workshop, etc.)</li>
